...
def datacollected():
    ...
    for a in flist:
        ...
        # create message box
        if (len(flist) != 0):
            messagebox.showinfo("Notification", "corona Record is saved {}".format(path2), parent=dapp)
...
dapp = Tk()
dapp.title("corona Virus Information")
dapp.geometry('800x500+200+80')
dapp.configure(bg='#8a0303')
dapp.iconbitmap('corona.ico')  # download only ico files
flist = []
path = ''

#### Labels
mainlabel = Label(dapp, text="Corona Virus Live Tracker", font=("Georgia", 30, "italic bold"), bg="#05897A", width=33
                  , fg="#732929", bd=5)
mainlabel.place(x=0, y=0)

label1 = Label(dapp, text="Country Name", font=("Verdana", 20, "bold"), bg="#05897A",fg="#732929",bd=1)
label1.place(x=15, y=100)

label2 = Label(dapp, text="Download File in ", font=("Verdana", 16, "bold"), bg="#05897A",fg="#732929")
label2.place(x=15, y=200)

cntdata = StringVar()
entry1 = Entry(dapp, textvariable=cntdata, font=("Verdana", 20, "bold"),bd=2, width=20)
entry1.place(x=280, y=100)

#### BUTTONS

Inhtml = Button(dapp, text="Html", bg="#2DAE9A", font=("arial", 15, "italic bold"), relief=RIDGE,
                activebackground="#05945B",
                activeforeground="white",bd=1, width=5, command=inhtmldownload)
Inhtml.place(x=400, y=200)

# Injson = Button(dapp, text="json", bg="#2DAE9A", font=("arial", 15, "italic bold"), relief=RIDGE,
#                 activebackground="#05945B",
#                 activeforeground="white", bd=5, width=5, command=injsondownload)
# Injson.place(x=300, y=260)

Inexcel = Button(dapp, text="Excel", bg="#2DAE9A", font=("arial", 15, "italic bold"), relief=RIDGE,
                 activebackground="#05945B",
                 activeforeground="white",bd=1, width=5, command=inexceldownload)
Inexcel.place(x=600, y=200)

Submit = Button(dapp, text="Submit", bg="#CB054A", font=("Weisshorn", 15, "bold"), relief=RIDGE,
